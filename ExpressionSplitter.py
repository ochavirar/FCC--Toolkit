...
def parentheses_indexes_search(exp):
    ...
    for i in range(0, len(exp), 1):  # Goes from 0 to the array's length-1
        if exp[i] == '(':  # If an opening parentheses is found then...
    ...
    closed_parentheses.reverse()  # Closing parentheses is reversed to pair the opening and closing indexes
...
def split_parentheses_expression(exp):
    ...
    open_parentheses, closed_parentheses = parentheses_indexes_search(exp)  # Gets opening an closing indexes
